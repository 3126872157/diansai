...
while(True):
    ...
    for i in range(img.width()):
        if img.get_pixel(i, 0) == 255:
            top_point_sum +=1
        if img.get_pixel(i, img.height()-1) == 255:
            bottom_point_sum +=1
    for j in range(img.height()):
        if img.get_pixel(0, j) == 255:
            left_point_sum +=1
        if img.get_pixel(img.width()-1, j) == 255:
            right_point_sum +=1
    if      top_point_sum > minCrossWidth     and     top_point_sum < maxCrossWidth      \
        and bottom_point_sum > minCrossWidth  and     bottom_point_sum < maxCrossWidth   \
        and left_point_sum > minCrossWidth    and     left_point_sum < maxCrossWidth     \
        and right_point_sum > minCrossWidth   and     right_point_sum < maxCrossWidth:
        uart.write("cross")
    else:
