...
def show_board():
    ...
    for i in range(0, len(real_edge_rect_corners) - 1):
        img.draw_line(int(real_edge_rect_corners[i][0]),int( real_edge_rect_corners[i][1]),
                      ...
                      color=(255, 0, 0))  # 红色线条
    img.draw_cross(int(img.width()/2),int(img.height()/2),(255,0,0),10,2)
    # 画出矩形的最后一个边
...
def color_recognition():
    ...
    while(totally_recognized_flag>0 and finger_block==0):
        ...
        if count!=0:
            ...
            finger_block_edge_block=int(finger_block_edge_block*3/standard_edge_rect_length)+1
            #print("sum:%d total:%d count:%d" % (sum,4*standard_edge_rect_length,count))
            #print("finger_block_edge_block:%d" % finger_block_edge_block)
            ...
            for i in range(9):
                if not_finger_flag[i]==0:
                    possible_finger_block.append(i+1)
                    #print("%d" % (i+1))
            if possible_finger_block:
                if len(possible_finger_block)==1:
